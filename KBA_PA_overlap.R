...
finfolder <- "C:/Users/Ashley.Simkins/Documents/SDG/KBA-PA overlap/KBA_PA_Overlap_rewritten/files_country_2019_rerun_countries" #folder where the files per country will be saved
setwd(folder)

tabmf <- read.csv("classif_KBAs_FINAL2019.csv")   ## file with types of kbas
isos <- read.csv("Iso_countries_2019.csv")            ## file with ISO codes; should be stored in the wkfolder specified above; no changes in 2019, so 2018 file used
...
for (x in 1:length(listcnts)){ #starts loop for all countries
  ...
  if (nrow(pa.c) == 0){ #finds all kbas with no protected area overlap - sets all output to 0 (0 overlap, no. of pas it overlaps with are 0, etc)
    areasov <- data.frame(SitRecID = kba.c$SitRecID, kba = NA, ovl = 0, year = 0, nPAs = 0, percPA = 0, ISO = country, COUNTRY = country.n) 
  }
  ...
  if (nrow(pa.c) > 0){
    ...
    if (length(ovkba) == 0){ #if there is no matrix produced, this is an error so set all outputs to error i.e. 9999
      areasov <- data.frame(SitRecID = NA, kba = NA, ovl = NA, year = NA, nPAs = NA, percPA = NA, ISO = country, COUNTRY = country.n)
    }
    ...
    if (length(ovkba) > 0){ #if there ARE overlaps between kbas and pas:
      ...
      pa.c$random <- F #not yet included in output
      if (min(pa.c$STATUS_YR) == 0){
        if (pa.c$STATUS_YR == 0){ #create a new column to identify any site that has had a year randomly allocated
          pa.c$random  <- T
        }
      ...
      #for (z in 1:length(kba.c)){ ## starts loop for all kbas in the country
        ...
        if (length(which(ovkba[ ,z] == T)) > 0)  ### when at least 1 pa overlaps with the kba
          ...
          if ("sf" %in% class(ovf) & length(yearspacz) > 0){
            ...
            areasov1 <- data.frame(SitRecID=kbaz$SitRecID, kba=akba, ovl=ovlz, year=year1, nPAs=nrow(ovf1)) #creates row in output table with this site overlap area and associated information within it #sets numbers to numeric not units (removes m^2)
            areasov1
            ...
            if (length(years) > 1){
              ...
              if (rema > 0.02){ #assuming 2% error in delineation of kbas compared to pas
                ...
                areasov1 <- rbind(areasov1,data.frame(SitRecID=kbaz$SitRecID, kba=akba, ovl=ovlz, year=year2, nPAs=nrow(ovf2)))
                areasov1
                }
              }
            }
        ...
        if (is.null(ovf) | !"sf" %in% class(ovf)){
          areasov1 <- data.frame(SitRecID=kbaz$SitRecID, kba=akba, ovl=NA, year=0, nPAs=0)  ## error in spatial overlap
        }
        }  ## ends loop for PAs overlapping with the KBA
      if (length(which(ovkba[ ,z] == T)) == 0){
        areasov1 <- data.frame(SitRecID=kbaz$SitRecID, kba=akba, ovl=0, year=0, nPAs=0)   ## no pas overlapping the kba
      }
...
final1 <- read_csv("finaltab.csv") #can restart from here
...
fields <- c("COUNTRY", "countryname","Developing","LDC","LLDC_SIDS","IPBES_region", "region", "SDG_Region","ISO_BL","ISO_SDG","SDG_Subregion") #Country and Region added back following output of last years list, add in "World_bank_class", "CMS_region", etc as appropriate
