  ...
  #outfile2 = "Output data for R Region KBAs.csv"
    ...
    for (k in ssets2run){
      ...
      for (z in 1:length(regs)){
        ...
        output <- mkdata(ibadat2$perprotected, ibadat2$country, ibadat2$year, ibadat2$random_year, ibadat2$siteid, j) #creates a list of years to make the error bars from (j repetitions so in this case a list of 100 possible years)
        
        # calculate the results   ## % of PAs fully covered
        alldata <- calcvals(ibadat2$perprotected, output)
